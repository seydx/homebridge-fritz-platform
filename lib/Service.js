        ...
        .then(response => {
          ...
          parseString(response.data,{explicitArray: false}, async (err, result) => {
            ...
            for(const action of result.scpd.actionList.action){
              ...
              this.actions[action.name] = async (vars, ident) => {
    
                try {
      
                  let identifier = ident.name||'Unknown Identifier';
      
                  let ping = await this._ping(identifier, this.config.type, this.config.host, this.config.port);
    
                  if(ping){
    
                    let res = await this._sendSOAPActionRequest(this.device,this.deviceInfo.controlURL,this.deviceInfo.serviceType,action.name,inArgs,outArgs,vars,ident);
    
                    if(res === 'challenge')
                      res = await this._sendSOAPActionRequest(this.device,this.deviceInfo.controlURL,this.deviceInfo.serviceType,action.name,inArgs,outArgs,vars,ident);
    ...
    return new Promise((resolve, reject) => {
      ...
      tcpp.probe(host, port, (err, available) => {
        ...
        if(available){
          ...
          this.log.debug(name + ': Ping ' + host + ':' + port + ' [' + type + '] --> SUCCESFULL');
          resolve(true);
