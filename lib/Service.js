...
class Service{
  ...
  tcp(name, type, service, action, host, port, callback){
    const self = this;
    if(this.log)this.log.debug(name + ': Ping ' + host + ':' + port + ' [' + type + '] - (' + service + ' | ' + action + ')')
    tcpp.probe(host, port, function(err, available) {
      if(available){
        if(self.log)self.log.debug(name + ': Ping ' + host + ':' + port + ' [' + type + '] - (' + service + ' | ' + action + ') --> SUCCESFULL')
      } else {
        if(self.log)self.log.debug(name + ': Ping ' + host + ':' + port + ' [' + type + '] - (' + service + ' | ' + action + ') --> ERROR');
      }
      callback(available);
    });
  }
