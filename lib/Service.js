...
class Service{
  ...
  tcp(name, type, service, action, host, port, callback){
    ...
    if(this.log){
      ...
      console.log(name + ': Ping ' + host + ':' + port + ' [' + type + '] - (' + service + ' | ' + action + ')')
    }
    tcpp.probe(host, port, function(err, available) {
      if(available){
        if(self.log){
          self.log.debug(name + ': Ping ' + host + ':' + port + ' [' + type + '] - (' + service + ' | ' + action + ') --> SUCCESFULL')
	} else {
          console.log(name + ': Ping ' + host + ':' + port + ' [' + type + '] - (' + service + ' | ' + action + ') --> SUCCESFULL')
	}
      } else {
        if(self.log){
          self.log.debug(name + ': Ping ' + host + ':' + port + ' [' + type + '] - (' + service + ' | ' + action + ') --> ERROR');
	} else {
	  console.log(name + ': Ping ' + host + ':' + port + ' [' + type + '] - (' + service + ' | ' + action + ') --> ERROR');
	}
      }
      callback(available);
    });
