...
const debug = require('debug')('FritzPlatformApi');
...
class Service{
  ...
  _ping(name, type, host, port){
    ...
    debug(name + ': Ping ' + host + ':' + port + ' [' + type + ']');
    ...
    return new Promise((resolve, reject) => {
      ...
      tcpp.probe(host, port, (err, available) => {
        ...
        if(available){
          ...
          debug(name + ': Pong ' + host + ':' + port + ' [' + type + '] --> SUCCESFULL');
          resolve(true);
          ...
          parseString(response.data,{explicitArray: false},(err, result) => {
            ...
            if (env['s:Header']) {
              ...
              if (header['h:Challenge']) {
                ...
                debug(device.deviceInfo.friendlyName + ': [Challenge] ' + ident.name + ' - '+ uri + ' - ' + JSON.stringify(requestInfo));
            ...
            debug(device.deviceInfo.friendlyName + ': [Authenticated] ' + ident.name + ' - '+ uri + ' - ' + JSON.stringify(requestInfo));
