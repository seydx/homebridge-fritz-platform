...
class Service{
  ...
  _parseSCPD(obj){
    ...
    let startUrl;
    self.host.match('myfritz') ? startUrl = 'https://' : startUrl = 'http://';
  ...
  _sendSOAPActionRequest(device,url,serviceType,action,inArguments,outArguments,initID,vars,callback){
    const self = this;
    if(!Object.keys(self.waitForAuth).length||initID==self.waitForAuth.initID){
      var head = '';
      if (device._auth.uid) {
      ...
      request(options,function(error, response, body) {
        if (!error && response.statusCode == 200) {
          ...
          if(body){
            ...
            error = {
              error: response ? response.statusMessage : 'No message',
              errorCode: response ? response.statusCode : 'No code',
              serviceType: serviceType,
              action: action,
              device: device.meta.friendlyName,
              host: device.meta.host
