...
class WatchNetworkService {
  constructor(devices, telegram, polling) {
    ...
    for (const [uuid, device] of this.devices) this.start(device);
  }
  ...
  async start(device) {
    ...
    try {
      ...
      let data = await device.fritzbox.exec('urn:LanDeviceHosts-com:serviceId:Hosts1', service, input);
      ...
      if (device.passed) {
        ...
        device.changedOn = false;
        if (newState) {
          if (this.Telegram) this.Telegram.send('network', 'on', device.name);
        } else {
          if (this.Telegram) this.Telegram.send('network', 'off', device.name);
        }
      }
    } catch (err) {
