...
class SmarthomeOutletLightbulbAccessory {
  ...
  async getService() {
    ...
    this.historyService = new this.FakeGatoHistoryService('energy', this.accessory, {
      storage: 'fs',
    ...
    serviceOutlet.getCharacteristic(this.api.hap.Characteristic.ResetTotal).on('set', (value, callback) => {
    ...
    if (
      ...
      if (!this.accessory.context.config.readOnly) {
        ...
        serviceOutlet.getCharacteristic(this.api.hap.Characteristic.On).on('set', (state, callback) => {
          Logger.info(
          ...
          setTimeout(() => {
            serviceOutlet.getCharacteristic(this.api.hap.Characteristic.On).updateValue(!state);
          }, 1000);
