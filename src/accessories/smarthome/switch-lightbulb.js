...
class SmarthomeSwitchLightbulbAccessory {
  ...
  getService () {
    ...
    if(!serviceSwitch){
      ...
      serviceSwitch = this.accessory.addService(this.api.hap.Service.Switch, this.accessory.displayName, this.accessory.context.config.subtype);
    }
    ...
    if(this.accessory.context.polling.timer && (!this.accessory.context.polling.exclude.includes(this.accessory.context.config.type) && !this.accessory.context.polling.exclude.includes(this.accessory.context.config.subtype) && !this.accessory.context.polling.exclude.includes(this.accessory.displayName))){
      ...
      if(!this.accessory.context.config.readOnly){
        ...
        serviceSwitch.getCharacteristic(this.api.hap.Characteristic.On)
          .on('set', this.handler.set.bind(this, this.accessory, this.api.hap.Service.Switch, this.api.hap.Characteristic.On, this.accessory.context.config.subtype, 'on'));
      ...
      if(!this.accessory.context.config.readOnly){
        ...
        serviceSwitch.getCharacteristic(this.api.hap.Characteristic.On)
          .on('get', this.handler.get.bind(this, this.accessory, this.api.hap.Service.Switch, this.api.hap.Characteristic.On, this.accessory.context.config.subtype, 'on'))
          .on('set', this.handler.set.bind(this, this.accessory, this.api.hap.Service.Switch, this.api.hap.Characteristic.On, this.accessory.context.config.subtype, 'on'));
