...
class CallmonitorContactAccessory {
    ...
    this.client.on('data', async (chunk) => {
      ...
      if (this.accessory.context.config.subtype == 'incoming' || this.accessory.context.config.subtype == 'group') {
        if (data[1] === 'ring') {
          ...
          if (this.accessory.context.config.incomingTo && this.accessory.context.config.incomingTo.length) {
            ...
            Logger.info(
              'Receiving new call from ' + message.caller + ' to ' + message.called,
              this.accessory.displayName
            );
      ...
      if (data[1] === 'connect') {
        if (this.call[data[2]]) {
          ...
          Logger.debug(
            'Connection established from: ' + message.caller + ' - to: ' + message.called,
            this.accessory.displayName
          );
        }
      }
      ...
      if (data[1] === 'disconnect') {
        if (this.call[data[2]]) {
          ...
          if (
            (this.accessory.context.config.incomingTo && this.accessory.context.config.incomingTo.length) ||
            (this.accessory.context.config.outgoingFrom && this.accessory.context.config.outgoingFrom.length)
          ) {
            if (
              this.accessory.context.config.incomingTo.includes(message.called) ||
              this.accessory.context.config.outgoingFrom.includes(message.caller)
            ) {
              Logger.info(
  ...
  updateAccessory(service, state, from) {
    ...
    if (from.caller) service.getCharacteristic(this.api.hap.Characteristic.Caller).updateValue(from.caller);

    if (from.called) service.getCharacteristic(this.api.hap.Characteristic.Called).updateValue(from.called);
  }
