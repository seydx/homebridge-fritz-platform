...
function FritzPlatform(log, config, api) {
  ...
  if (this.config.devices) {
    ...
    this.config.devices.map((device) => {
      if (device.master && device.host) {
        ...
        this.masterDevice.fritzbox = new Fritzbox({
          ...
          url: 'http://' + device.host + ':' + (device.port || 49000),
          tr064: device.tr064,
          igd: device.igd,
          autoSsl: device.ssl,
        });
    ...
    if (!this.masterDevice) {
      Logger.warn(
        'WARNING: There is no master router defined! Please check if "master" is enabled and "host" ist set in config.'
      );
      return;
    }
    ...
    this.config.devices.forEach((router) => {
      ...
      if (!router.name) {
        ...
        Logger.warn(
          'There is no or no valid connection type configured for this router. Setting it to default: "dsl"',
          router.name
        );
      ...
      router.password = router.password || false;
      ...
      if (!error) {
        ...
        if (this.devices.has(uuid)) {
          ...
          if (!router.hide) this.devices.set(uuid, router);

          if (router.options) {
            let validChars = ['wifi_2ghz', 'wifi_5ghz', 'wifi_guest', 'wps', 'dect', 'aw', 'deflection', 'led', 'lock'];

            const switches = Object.keys(router.options).filter(
              (extra) => validChars.includes(extra) && router.options[extra] === 'switch'
            );
    ...
    this.config.smarthome.forEach((device) => {
      ...
      if (!device.group) {
        if (!device.name) {
          ...
          Logger.warn(
            'There is no or no valid type configured for this smarthome device. This device will be skipped.',
            device.name
          );
          error = true;
        }
        ...
        if (!device.name) {
          ...
          Logger.warn(
            'There is no or no valid type configured for this grouped smarthome device. This device will be skipped.',
            device.name
          );
          error = true;
        }
      ...
      if (!error) {
        if (!device.group) device.ain = device.ain.replace(/\s/g, '');
  ...
  if (this.config.wol) {
    this.config.wol.forEach((device) => {
      ...
      if (!device.name) {
        ...
        Logger.warn(
          'The address for this WOL device is not a valid MAC address. This device will be skipped.',
          device.name
        );
        error = true;
      }
  ...
  if (this.config.network) {
    this.config.network.forEach((device) => {
      ...
      if (!device.name) {
        ...
        Logger.warn(
          'There is no address configured for this Network device. This device will be skipped.',
          device.name
        );
        error = true;
      } else if (!this.validMAC.test(device.address) && !this.validIP.test(device.address)) {
        Logger.warn(
          'The address for this WOL device is not a valid IP/MAC address. This device will be skipped.',
          device.name
        );
        error = true;
      }
      ...
      if (!error) {
        ...
        if (this.network.has(uuid)) {
          Logger.warn(
            'Multiple network devices are configured with this name. Duplicate devices will be skipped.',
            device.name
          );
        } else {
  ...
  if (config.extras && Object.keys(config.extras).length) {
    ...
    Object.keys(config.extras)
      .filter(
        (device) =>
          Object.keys(config.extras[device]).length &&
          config.extras[device].active &&
          config.extras[device].accType === 'characteristic'
      )
      .map((device) => {
    ...
    let extras = Object.keys(config.extras).filter(
      (device) =>
        Object.keys(config.extras[device]).length &&
        config.extras[device].active &&
        config.extras[device].accType === 'switch'
    );
    ...
    this.extrasAsCharacteristics = Object.keys(this.extrasAsCharacteristics).length
      ? this.extrasAsCharacteristics
      : false;
  }
  ...
  if (config.options && Object.keys(config.options).length) {
    ...
    this.polling = {
      timer:
        config.options.polling && !isNaN(parseInt(config.options.polling.timer))
          ? config.options.polling.timer < 1
            ? false
            : config.options.polling.timer * 1000
          : 10000,
      exclude:
        config.options.polling && config.options.polling.exclude && config.options.polling.exclude.length
  ...
  if (!this.polling.exclude.includes('wakeup')) this.polling.exclude.push('wakeup');

  if (!this.polling.exclude.includes('alarm')) this.polling.exclude.push('alarm');

  if (!this.polling.exclude.includes('phoneBook')) this.polling.exclude.push('phoneBook');

  if (!this.polling.exclude.includes('broadband')) this.polling.exclude.push('broadband');
  ...
  if (
    config.callmonitor &&
    config.callmonitor.active &&
    config.callmonitor.ip &&
    this.validIP.test(config.callmonitor.ip)
  ) {
    ...
    this.config.callmonitor.incomingTo =
      this.config.callmonitor.incomingTo && this.config.callmonitor.incomingTo.length
        ? this.config.callmonitor.incomingTo
        : [];

    this.config.callmonitor.outgoingFrom =
      this.config.callmonitor.outgoingFrom && this.config.callmonitor.outgoingFrom.length
        ? this.config.callmonitor.outgoingFrom
        : [];
  ...
  if (this.network.size) new WatchNetwork(this.network, this.Telegram, this.polling);

  //listener to close the callmonitor
  this.api.on('shutdown', () => {
    if (this.Callmonitor) this.Callmonitor.stop();
  });
...
FritzPlatform.prototype = {
  didFinishLaunching: async function () {
    ...
    if (this.polling.timer) this.handler.poll(this.accessories);
  },
  ...
  setupAccessory: async function (accessory, device) {
    ...
    if (device.type === 'router') {
      ...
      manufacturer =
        this.masterDevice.manufacturer && this.masterDevice.manufacturer !== ''
          ? this.masterDevice.manufacturer
          : 'Homebridge';
