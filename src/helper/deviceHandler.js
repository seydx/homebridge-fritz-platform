...
module.exports = (
  ...
  async function get(accessory, service, characteristic, target, config, callback) {
    Logger.debug(
      (typeof callback !== 'function' ? 'Polling' : 'Getting') + ' state (' + target + ')...',
    ...
    switch (target) {
      case 'smarthome-switch-lightbulb': {
        ...
        if (accessory.context.config.brightness)
          accessory.context.config.brightnessValue = accessory
            ...
            .getCharacteristic(api.hap.Characteristic.Brightness).value;
        ...
        if (accessory.context.config.color) {
          ...
          accessory.context.config.tempValue = accessory
            .getService(api.hap.Service.Lightbulb)
            .getCharacteristic(api.hap.Characteristic.ColorTemperature).value;
        }

        try {
          if (!smarthomeGroupList) await this.refreshSmarthome(false, true);
          ...
          if (device) accessory.context.config.ain = device.ain;
          ...
          if (device && device.online && device.switch) {
            ...
            if (accessory.context.config.energy && device.powermeter) {
              ...
              accessory.getService(service).getCharacteristic(api.hap.Characteristic.Volts).updateValue(voltage);

              accessory.getService(service).getCharacteristic(api.hap.Characteristic.Amperes).updateValue(ampere);
            }
          }
          ...
          state = handleError(
            ...
            target,
            err,
            typeof callback === 'function' ? { get: true } : { poll: true }
          );
        }

        accessory.getService(service).getCharacteristic(characteristic).updateValue(state);

        accessory.getService(api.hap.Service.Lightbulb).getCharacteristic(characteristic).updateValue(bulbState);
        ...
        if (accessory.context.config.color) {
          if (temp !== null && temp !== undefined) {
            ...
            if (changed) {
              ...
              let color =
                api.versionGreaterOrEqual && api.versionGreaterOrEqual('v1.3.0-beta.23')
      ...
      case 'smarthome-lightbulb': {
        ...
        try {
          if (
            (!accessory.context.config.group && !smarthomeList) ||
            (accessory.context.config.group && !smarthomeGroupList)
          )
        ...
        if (
          ...
          accessory.getService(service).getCharacteristic(api.hap.Characteristic.Brightness).updateValue(brightness);
        }
        ...
        if (accessory.context.config.color) {
          if (temp !== null && temp !== undefined) {
            ...
            if (changed) {
              ...
              accessory.getService(service).getCharacteristic(api.hap.Characteristic.Hue).updateValue(color.hue);
          ...
          if (hue !== null && hue !== undefined && sat !== null && sat !== undefined) {
            ...
            accessory.getService(service).getCharacteristic(api.hap.Characteristic.Hue).updateValue(hue);

            accessory.getService(service).getCharacteristic(api.hap.Characteristic.Saturation).updateValue(sat);
          }
        }
      ...
      case 'smarthome-temperature': {
        ...
        try {
          if (!smarthomeList) await this.refreshSmarthome(false, true);
      ...
      case 'smarthome-thermostat': {
        let active = accessory
          ...
          .getCharacteristic(api.hap.Characteristic.Active).value;
        ...
        try {
          ...
          if (device && device.online && device.thermostat) {
            ...
            if (device.temperature) currentTemp = device.temperature.value || currentTemp;
            ...
            accessory.getService(service).getCharacteristic(api.hap.Characteristic.Active).updateValue(active);
      ...
      case 'dect': {
        ...
        try {
          ...
          let body = await lua.request(
            ...
            accessory.context.config.fritzbox.url.hostname,
            '/data.lua',
            'dect_activ'
          );
      ...
      case 'led': {
        ...
        try {
          ...
          //old fw
          if (body && body.data && body.data.led_display) state = parseInt(body.data.led_display) === 0 ? true : false;
        } catch (err) {
          state = handleError(
            accessory,
            state,
      ...
      case 'lock': {
        ...
        try {
          ...
          if (body && body.data) state = body.data.keylock_checked || false;
        } catch (err) {
          state = handleError(
            accessory,
            state,
      ...
      case 'broadband': {
        ...
        if (ul) {
          accessory.getService(service).getCharacteristic(api.hap.Characteristic.Upload).updateValue(ul);
        }
    ...
    switch (target) {
      case 'smarthome-switch-lightbulb': {
        let bulbState = accessory
          .getService(api.hap.Service.Lightbulb)
          .getCharacteristic(api.hap.Characteristic.On).value;
        ...
        try {
          ...
          if (accessory.context.config.group && !accessory.context.config.ain) {
            ...
            let device = smarthomeList.find((device) => device.name.includes(accessory.displayName));
      ...
      case 'wol': {
        try {
          ...
          setTimeout(() => {
            accessory.getService(service).getCharacteristic(characteristic).updateValue(false);
          }, 1000);
        }
          ...
          if (state) {
            ...
            if (config.duration && parseInt(config.duration) > 0) {
              ...
              accessory.context.alarmTimeout = setTimeout(() => {
                ...
                if (value) {
                  accessory.getService(service).getCharacteristic(characteristic).setValue(false);
                }
              }, duration * 1000);
            }
          } else {
      ...
      case 'phoneBook': {
        if (!state) {
          ...
          setTimeout(() => {
            accessory.getService(service).getCharacteristic(characteristic).updateValue(true);
          }, 1000);
        } else {
          try {
            let lkz, okz;
            ...
            if (masterDevice.countryPrefix) {
              ...
              if (prefix.includes('+')) lkz = prefix.split('+')[1];
            ...
            let blacklists = config.blacklists && config.blacklists.length ? config.blacklists : [];
                    ...
                    if (numbers.length) {
                      ...
                      for (const number of numbers) {
                        ...
                        if (telnr.startsWith(lkz1) || telnr.startsWith(lkz2)) {
                          ...
                          if (telnr.startsWith(okz1)) telNumbers.push(telnr.replace(okz1, ''));
            ...
            Logger.info(
              'Storing phonebook results to ' + configPath + '/fritzbox/phonebook.json',
              accessory.displayName
            );
            ...
            if (blackBook.length) {
              Logger.info(
                'Storing blackbook results to ' + configPath + '/fritzbox/blackbook.json',
                accessory.displayName
              );
  ...
  async function change(accessory, target, replacer, historyService, value) {
    if (value.oldValue !== value.newValue) {
      switch (target) {
        ...
        case 'network': {
          if (Telegram) Telegram.send('network', value.newValue ? 'on' : 'off', replacer);

          break;
        }

        case 'alarm': {
          if (Telegram) Telegram.send('alarm', value.newValue ? 'activated' : 'deactivated', replacer);
  ...
  async function refreshDevices(accessories) {
    if (devices.size) {
      ...
      for (const [uuid, device] of allDevices) {
        if (
          !polling.exclude.includes(device.subtype) &&
          !polling.exclude.includes(device.type) &&
          !polling.exclude.includes(device.name)
        ) {
          const accessory = accessories.find(
            (curAcc) => curAcc.UUID === uuid || curAcc.UUID + '-' + device.subtype === uuid
          );
  ...
  async function refreshHosts(accessories) {
    ...
    if (poll) {
      try {
        ...
        const data = await masterDevice.fritzbox.exec(
          'urn:LanDeviceHosts-com:serviceId:Hosts1',
          'X_AVM-DE_GetHostListPath'
        );
        const uri =
          (ssl ? 'https://' : 'http://') +
          masterDevice.fritzbox.url.hostname +
          ':' +
          (ssl ? '49443' : '49000') +
        ...
        for (const [uuid, device] of presence) {
          if (
            ...
            if (accessory.displayName !== 'Anyone') {
              let accService =
                device.accType === 'occupancy' ? api.hap.Service.OccupancySensor : api.hap.Service.MotionSensor;
              let accCharacteristic =
                device.accType === 'occupancy'
  ...
  async function refreshSmarthome(accessories, poll) {
    ...
    if (poll) {
      try {
        ...
        if (!Array.isArray(deviceList)) deviceList = [deviceList];

        Logger.debug(deviceList, 'SmartHome Devices');

        if (groupList) Logger.debug(groupList, 'SmartHome Groups');

        smarthomeList = deviceList
          .map((device) => {
            ...
            if (
              ...
              switch (device.subtype) {
                case 'smarthome-switch-lightbulb':
                  await get(
                    accessory,
                    device.energy ? api.hap.Service.Outlet : api.hap.Service.Switch,
  ...
  function getValidColor(accessory, hue, sat) {
    ...
    sat = Math.round(
      (sat ||
        accessory.getService(api.hap.Service.Lightbulb).getCharacteristic(api.hap.Characteristic.Saturation).value) *
        2.55
    );
  ...
  function handleError(accessory, state, target, err, action) {
    ...
    if (
      ...
      Logger.warn(
        'Could not process the request during ' + mode + ' state' + ' (' + target + ')',
        accessory.displayName
      );
    } else {
